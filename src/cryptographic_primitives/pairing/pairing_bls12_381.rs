...
pub mod tests {
    ...
    fn powers_of_g1_and_g2() {
        ...
        let res_a_power = PairingBls::compute_pairing(&res_mul_a, &b);
        let res_b_power = PairingBls::compute_pairing(&a, &res_mul_b);
    ...
    fn powers_of_g1_and_gt_eq() {
        ...
        let scalar_factor: FE1 = ECScalar::new_random();
        let res_mul_a: GE1 = a.scalar_mul(&scalar_factor.get_element());
        let gt_from_a_power = PairingBls::compute_pairing(&res_mul_a, &b);
    ...
    fn powers_of_g2_and_gt_eq() {
        ...
        let scalar_factor: FE1 = ECScalar::new_random();
        let res_mul_b: GE2 = b.scalar_mul(&scalar_factor.get_element());
        let gt_from_a_power = PairingBls::compute_pairing(&a, &res_mul_b);
        let gt_direct_power = PairingBls::compute_pairing(&a, &b) * scalar_factor.get_element();
        assert_eq!(gt_direct_power, gt_from_a_power);
    }
}
